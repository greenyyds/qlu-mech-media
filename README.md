...
| **风采展示** | 首页全宽自动轮播（宇树官网风格）：4 张部门活动照片（**WebP 压缩，共 369KB**）、自动播放、箭头/圆点/键盘切换、触摸滑动 |
| **内部教程** | `#/tutorials` 教程中心：摄影技术（**分步引导式九章教程**，内容源自部门 docx）、新闻与图像处理/摄像技术/新媒体运营（筹备中）；入口：导航 + 首页卡片 |
| 常用链接 | 媒体链接 ×3 + 工具链接 ×4（醒图/秀米/剪映/DeepSeek，**品牌色图标**），新窗口打开 |
| **AI 新闻初稿生成** | **智谱 GLM 双模式**（快速 = glm-4-flash / 深度 = glm-4.5-flash，默认深度）：11 项选项、**流式打字机输出**、复制全文、重新生成、最近生成记录（本地保存 5 条） |
| 任务规划 | 三列看板（待办 / 进行中 / 已完成），添加 / 编辑 / 删除 / 拖拽换列 / 进度条；**云端共享**（或本地模式）；**操作零等待（乐观更新，失败自动回滚）** |
| 值班表 | 页面内可编辑（每日 ≤4 人），**云端共享**（或本地模式），可重置示例；**乐观更新零等待** |
| **离线自愈** | 云端异常时：顶部横幅显示**具体原因**（匿名登录/集合/网络或安全域名）+ 数据暂存本机 + **每 30 秒自动重试恢复**，无需手动操作 |
| **意见反馈页** | `#/feedback` 二级页面：技术/设计 + 工作/交际两分类、匿名或实名（**重构后开关组件**）、公开列表（分类筛选）、**预留密码保护开关** |
...
- ✅ 数据共享：双模式架构（云端/本地/离线降级），状态徽标明确提示；**线上「云端共享」实测生效**（Web 安全域名已正确配置）
- ✅ 风采轮播：4 图全宽自动播放、箭头/圆点/键盘/触摸切换、**WebP 压缩后共 369KB**（加载提速 2.5 倍）
- ✅ 意见反馈：二级页路由、两分类、匿名/实名、公开列表、密码预留
- ✅ 备份导出：一键下载 JSON
- ✅ 操作体验：任务/值班表**乐观更新零等待**，云端失败自动回滚
- ✅ 离线自愈：原因诊断横幅 + 30 秒自动重连 + 数据自动刷新
- ✅ 冒烟测试 50 项全绿、构建零报错、控制台零报错
- ✅ 性能：SDK 动态加载，本地模式主包 234KB（gzip ~68KB）；图片总 369KB
