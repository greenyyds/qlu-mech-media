...
/**
 * 摄影技术教程（#/tutorials/photography）—— 分步引导式
 * - 章节进度条 + 左右翻页 + 键盘方向键 + 章节圆点
 * - 章节内容：要点清单 / 实战提示高亮卡 / 配图 / 参数速查表
 * - 内容源：素材/摄影教程/摄影文本.docx（忠于原文）
 */
export default function PhotographyTutorial() {
  const chapters = photographyTutorial.chapters
  const [index, setIndex] = useState(0)
  const total = chapters.length
  const chapter = chapters[index]

  useEffect(() => {
    document.title = `${photographyTutorial.title} · 内部教程`
    return () => {
      ...
      <div className="mx-auto max-w-4xl px-5 md:px-8">
        ...
        <Reveal>
          ...
          <h1 className="mt-4 text-3xl font-bold tracking-[-0.02em] text-ink md:text-4xl">
            {photographyTutorial.title}
          </h1>
          <p className="mt-2 text-[14px] text-secondary">{photographyTutorial.subtitle}</p>
        </Reveal>
